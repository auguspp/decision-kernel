...
Status: IMPLEMENTED OPT-IN BATCHING AND EXACT PLAN EXECUTION / SYNTHETIC ACCEPTANCE REQUIRED / NO NEW LIVE SOURCE OR MARKET PROOF.
...
After each 20-second pacing interval, check the existing saved-day window and monotonic request clock immediately before calling the transport. Recheck at response receipt before storage or further requests. An invocation that starts inside the window cannot finish its wait outside it and still issue requests. If the response itself arrives after the window, retain the failed attempt metadata but issue no further requests. Preserve an invalid received-time claim instead of repairing it: when clocks reverse, the ordinary execution verifier rejects that receipt as temporally invalid rather than certifying a clean failure.

Regression-first proof: PR #255's tests-only head `28e9275cf9d8c92ff102b45d61161502576ca8be` against the merged #254 code produced **13 failed, 1376 passed** in CI `34016516585` / job `101441119351`. All 1374 pre-existing tests still passed; two new cases already passed. The failures exposed missing decoded-response guards and seven simulated requests continuing across time boundaries before late rejection. The tests use fake credentials and synthetic transport only, not a real secret disclosure or live stale request. The large-body test was then given a compact test ID to avoid rendering its payload in test names; its rejection condition was not weakened. Final fixed CI is recorded in the PR separately. Positive full-probe cases remain required, including valid final weekend seconds.
