...
Execution follow-up: PR #243 connects this unchanged module to the existing capture trial. See `docs/theme-source-capture-v0.md` for execution rules and `docs/theme-source-capture-proof-2026-09-06.md` for the actual previously retained Muyuan-source sample, seven requests and downloaded evidence. This does not establish a natural incoming feed or change the offline module's zero-request boundary.
...
The returned plan is compatible with the existing probe input's `plan` field. Original catalog captures plus every later exact response must then pass `build_theme_probe`; no invented empty captures or successful validation marker substitutes for execution. Unit tests run that downstream builder using synthetic price/member fixtures and verify unchanged calculations. The original PR #242 slice did not execute those plans. The later PR #243 collector now retains the original source input/projection before the first detail request and verifies one real existing-source sample, as documented above. Automatic source ingestion, daily dedup state, accepted-source registry updates and a natural new-source run remain unestablished; the standalone module still performs no network calls.
