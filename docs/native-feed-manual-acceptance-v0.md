...
The default `native-history-mode=isolated` retains the v0 behavior below. Opt-in `initialize`/`continue` workflow history is specified in `docs/native-feed-history-workflow-v1.md`; its snapshot commands reuse the original consumer directories described in `docs/native-feed-consumer-history-v0.md`. Implementation and real cloud-history acceptance remain distinct.
...
A default isolated invocation declares `EXPLICIT_EMPTY_HISTORY_FOR_ISOLATED_ACCEPTANCE_NOT_DAILY_CONSUMPTION`. This is a deliberately isolated compatibility study, not resumption of the registered scan/execution history. It never clears a prior store or resets RSS. Repeating this study can rescan the same post-baseline descriptions and, with explicit execution enabled, make another market attempt. Do not describe it as cross-run exactly-once delivery or use repeated studies as independent prospective samples. Explicit tracked modes use the separate pinned-history contract linked above; they are never selected implicitly.
