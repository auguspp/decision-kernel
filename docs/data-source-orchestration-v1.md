...
官方接口合同：Tushare文档311、312、292、106、107。本次2026-09-26实读官方292列示每晚19—22点更新；此前本文件列示的21:15及次日09:15未获本轮原文支持，不再作为当前接口合同。现有17:20/20:10执行仍不保证全部晚间报告已经取得。第7节原批次只交付离线解释；第8节是采集接续前驱，第9节修正真实Relay参数差异。真正分页及更广市场完整性未由单页返回建立。

此解释仅使用原件与标准库和既有canonical工具；不新增HTTP实现、凭证入口、provider路由、定时或费用。退出时随Relay消费者一并移除解释模块及专属回归，保留原件、旧解释和研究引用。


## 8. 原客户端保留与有界采集接续（计划revision 2前驱）
...
## 9. 真实参数修复与七个独立报告日（计划revision 3）

#593首轮正式Relay-only `36240623538` 的原件明确返回 `HTTP400 / invalid_params`：此中转要求report_rc至少给出ts_code或report_date。仅start_date/end_date虽是官方接口的参数，却不能推断第三方中转接受同一组合。原revision2失败、请求参数和capture hash原样保留，不能重写成当时已经成功。

后继revision3保持“实际取得日及此前6个日历日”的范围，改为按**七个明确report_date**逐日请求，每日limit=3000，从新到旧；不尝试通配代码、逗号日期、备用host或并发绕过参数限制。每页独立保留原件、日期、成功/失败/空页和覆盖缺口；合并阅读保留页偏移及每行的请求索引/源行号，不把最后一天覆盖前六天，也不把多预测期行数当作独立报告数。空日不是无事件；每日期一页仍不保证分页或全市场完整。

正常Relay采集为4个其他接口加7个报告日，最多11个逻辑请求、22次原客户端HTTP尝试；总原件20MiB、单体4MiB、现有10分钟job上限和逐页checkpoint不变。时间/容量不足可能留下未执行日，不能签完整。原主采每日上限、Relay已启动job每日累计3次和失败停止不变；无新增日程或持续费用。

显式参数修复可在同一workflow使用 `relay-only=true` 加 `relay-reports-only=true`，仍必须通过原exact-main/原state/原source/每日3次门禁，不能单独使用后者。此时只发七个report_rc请求，不重新请求原12族、名录、两种龙虎榜或未改变合同的排队游资明细。它是当前故障的有界维护入口，不是通用接口选择器或新的定时任务。

正常读取同时检查新manifest的reports_only与同run原control一致。窄修复的当前报告与前次其他补充**分开显示、分开截止和原件身份**；同R `relay-previous.json`保存先前可读补充，不把旧名录/榜单重新命名为新取得。前次解释无法恢复时留下原R定位，当前报告和主历史继续可读；重复publisher沿同一明确前驱，不形成无限历史扫描。自然全范围后继有自己的完整范围，不继承窄修复的READY为全市场无缺口。

原有限HTTP客户端blob保持 `d2ee02a81648eafe7204a47e7f8e41b56c3c48fd`。Reuse沿已有plan/capture/replay、单页解释、旧原件读取、workflow/controller及既有Tushare SDK审阅；只薄适配实际第三方参数差异。依据：正式原件raw-02-1.json、供应商0.5.61调用说明、[官方report_rc](https://tushare.pro/document/2?doc_id=292)。接口失败原因以上述实际返回为准，不推测更深上游原因。

退出时在原Relay整体退出清单中同步移除此输入、逐日计划/聚合及专属测试，保留旧revision1/2/3原件和仍被消费的历史解释。工程完成、真实请求结果、正常发布/同R读回、Quick实际消费分别记录，不以fixture或接口数代替生产签收。
