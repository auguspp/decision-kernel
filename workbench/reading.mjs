...
export function references(payload) {
  ...
  function visit(value, location = '') {
    ...
    if (!Array.isArray(value) && typeof value.read_path === 'string') {
      ...
      } else found.set(value.read_path, {...value, location});
    }
...
export async function readHealth(fetcher = globalThis.fetch) {
  ...
  require(item.number === 581 && item.html_url === `https://github.com/${REPO}/issues/581` &&
    typeof item.body === 'string', 'WRONG_HEALTH_ISSUE');
...
export async function readQuick(fetcher = globalThis.fetch) {
  ...
  const items = rows.filter(row => typeof row.body === 'string' &&
    /^## DAILY HOSTED QUICK \d{4}-\d{2}-\d{2}(?:\s|$)/.test(row.body)).map(row => {
      require(Number.isSafeInteger(row.id) && row.id > 0 &&
        row.html_url === `https://github.com/${REPO}/issues/575#issuecomment-${row.id}` &&
        Number.isFinite(Date.parse(row.updated_at)) && Number.isFinite(Date.parse(row.created_at)),
      'INVALID_QUICK_IDENTITY');
      return {id: row.id, text: row.body, url: row.html_url,
        createdAt: row.created_at, updatedAt: row.updated_at};
    });
  require(new Set(items.map(row => row.id)).size === items.length, 'MOVING_COMMENT_PAGES');
  items.sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt) || b.id - a.id);
  return deepFreeze({items, pages, observedAt: new Date().toISOString(),
