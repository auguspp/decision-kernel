...
def _questions(row):
    ...
    if len({v['kind'] for v in row['origins']}) > 1:
        questions.append('两种发现来路落在同一公司：分别核对日期和业务事实，不把来源重叠当成独立投资确认。')
...
def build(baseline, *, sector_result, sector_source, institution_report, institution_source,
    ...
    payload = {'version': VERSION, 'base_reading_hash': baseline['reading_hash'],
...
def render(report):
    p = report['projection']
    model.check(report['projection_hash'] == canonical_hash(p) and p['version'] == VERSION
    ...
    parts = ['<!doctype html><html lang="zh-CN"><meta charset="utf-8">',
        ...
        '<h1>公司发现与研究上下文</h1>',
        f'<p><b>{c["distinct_companies"]} 家保存线索</b>：行业 {c["sector_companies"]} 家，机构 {c["institutional_companies"]} 家，重叠 {c["overlap_companies"]} 家。</p>',
