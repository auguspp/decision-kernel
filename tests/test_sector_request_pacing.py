...
def test_only_approved_sector_capture_steps_enable_pacing():
    ...
    # Human-approved one-off recovery reuses the same pacing, not an opt-out.
    recovery = (root / ".github/workflows/sector-recovery-once.yml").read_text()
    step = recovery.split("- name: Acquire missing completed index sessions once", 1)[1].split("- name: Rebuild the candidate offline", 1)[0]
    assert "HITHINK_SECTOR_REQUEST_PACING: '1'" in step
    assert recovery.count("HITHINK_SECTOR_REQUEST_PACING") == 1
    assert "workflow_dispatch:" in recovery and "  schedule:" not in recovery and "  push:" not in recovery
    assert "timeout-minutes: 150" in recovery and "timeout-minutes: 140" in step
    assert "group: sector-radar-prospective-state" in recovery and "cancel-in-progress: false" in recovery
