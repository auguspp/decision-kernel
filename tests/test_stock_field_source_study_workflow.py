...
def test_credential_is_only_in_source_step_and_offline_check_always_runs():
    text = WORKFLOW.read_text(encoding='utf-8')
    assert text.count('secrets.HITHINK_FINANCE_API_KEY') == 1
    source = text.split('- name: Collect targeted events')[1].split('- name: Verify retained')[0]
    assert 'secrets.HITHINK_FINANCE_API_KEY' in source
    verify = text.split('- name: Verify retained')[1].split('- name: Retain complete')[0]
    assert 'if: always()' in verify
    assert 'build_plan(source) == plan' in verify
