"""Build the reviewed production slice; temporary preparation, never shipped to main."""
from pathlib import Path
import subprocess

root=Path.cwd()
def patch(path,old,new):
    p=root/path; value=p.read_text(); assert old in value,(path,old[:80]); p.write_text(value.replace(old,new,1))

patch('src/decision_kernel/runtime/industry_fundamentals_reading.py',
...
patch('src/decision_kernel/runtime/industry_fundamentals.py',
      "number(previous[pos]) not in (None, '0'):",
      "number(previous[pos]) is not None and Decimal(number(previous[pos])) != 0:")
# Attach next to the original industry seam, not inside the commodity reader.
patch('src/decision_kernel/runtime/current_state_delivery_with_odds_watch.py',
...
p=root/'.github/workflows/radar-industry-breadth.yml'
s=p.read_text(); original=s[s.index('  public-context:'):]
job=original.replace('  public-context:', '  industrial-fundamentals:',1)
job=job.replace('timeout-minutes: 5','timeout-minutes: 7')
job=job.replace('public-ci.json','fundamentals-ci.json')
job=job.replace('Capture bounded public context','Capture bounded industrial fundamentals independently')
job=job.replace('decision_kernel.runtime.easy_stock_capture','decision_kernel.runtime.industry_fundamentals')
job=job.replace('easy-stock-context','industry-fundamentals')
job=job.replace('Upload public context original attempts','Retain independent industrial capture with per-source gaps')
p.write_text(s+'\n'+job)

p=root/'docs/industry-fundamentals-v1.md'
p.write_text('''# 产业雷达：真实经济变量日常观察 v1
...
''')
subprocess.run(['python','-m','compileall','-q','src/decision_kernel/runtime/industry_fundamentals.py','src/decision_kernel/runtime/industry_fundamentals_reading.py'],check=True)
