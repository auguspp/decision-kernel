#!/usr/bin/env python3
"""One-request same-source HTTP-v4 qualification for 884189.TI.

This reuses the prior bounded v4 probe shape. It makes exactly one public THS
request, compares the retained 2026-09-07..09 overlap, retains raw bytes/hash,
and has no HiThink, production-state, event, Research, Odds, or Action authority.
"""
import hashlib
import json
import sys
...
URL = "http://d.10jqka.com.cn/v4/line/bk_884189/01/2026.js"
...
try:
    ...
    body = b""
    http_status = None
    request_error = {"type": type(exc).__name__, "message": str(exc)}

(OUT / "884189.js").write_bytes(body)
result = {
    "schema_version": 1,
