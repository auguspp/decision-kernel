...
原有研究包价格复核：完成 4/4，无法判断 0。Watch：已启用 5，已完成判断 5；确认触界 0，未触界 5，无法判断 0。



# Attention Inbox

生成时间：2026-09-25T08:54:16.451493+00:00
...
**当前状态：** Research 已存在 · `ACCEPTABLE_ODDS` · ¥1237.00 · 行情 2026-09-24T15:00:00+08:00
...
- **三花智控 002050** — ¥36.08 — `INSUFFICIENT_ODDS` — 重新值得看：`ACCEPTABLE_ODDS ≤ ¥29.56`
<details>
<summary>为什么安静</summary>

- 当前：期望收益 `4.1%` · 正收益概率 `60%`
- `ACCEPTABLE_ODDS` 要求：期望收益 ≥ `27%` · 正收益概率 ≥ `55%`
- 重新值得看：`ACCEPTABLE_ODDS ≤ ¥29.56`（距当前价约 `-18.1%`）
...
- **中国神华 601088** — ¥48.03 — `INSUFFICIENT_ODDS` — 重新值得看：`ACCEPTABLE_ODDS ≤ ¥35.37`
<details>
<summary>为什么安静</summary>

- 当前：期望收益 `-6.5%` · 正收益概率 `25%`
- `ACCEPTABLE_ODDS` 要求：期望收益 ≥ `27%` · 正收益概率 ≥ `55%`
- 重新值得看：`ACCEPTABLE_ODDS ≤ ¥35.37`（距当前价约 `-26.4%`）
- Frozen scenarios：base 50% → ¥44.80 · downside 25% → ¥31.20 · upside 25% → ¥58.90

</details>
- **兆易创新 603986** — ¥383.77 — `INSUFFICIENT_ODDS` — 重新值得看：`ACCEPTABLE_ODDS ≤ ¥330.69`
<details>
<summary>为什么安静</summary>

- 当前：期望收益 `13.7%` · 正收益概率 `70%`
- `ACCEPTABLE_ODDS` 要求：期望收益 ≥ `32%` · 正收益概率 ≥ `55%`
- 重新值得看：`ACCEPTABLE_ODDS ≤ ¥330.69`（距当前价约 `-13.8%`）
